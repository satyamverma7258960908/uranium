...
const createReview = async (req, res) => {
 try {
    ...
    const obj = {
       ...
       if (!bookId){
       return res.status(400).send({status:false, msg: "bookId not given" })
       }
       obj.bookId=bookId.trim()
      ...
      return res.status(400).send({ status: false, msg: "reviewedBy must be valid it should not contains numbers" });
        ...
        if (!validatefeild(review)) {
         ...
         if (isDeleted){
            ...
            if(isDeleted==true){
            return res.status(400).send({status:false,message:"isDeleted must be false while giving reviews"});
            }}
